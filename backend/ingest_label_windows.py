...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--segments-file", required=True,
                   help="CSV or JSON lines file with segment definitions")
...
def main():
    ...
    reports = load_self_reports(
        args.self_reports_file) if args.self_reports_file else None
    ...
    for fp in files:
        ...
        subject_id = args.subject_id or flat.get("subject_id", "S1")
        ...
        seg_label, seg_phase = assign_segment_label(
            mid_ts, segments, flat["subject_id"], flat["session_id"])
        ...
        if reports is not None:
            override = find_self_report_override(
                mid_ts, reports, flat["subject_id"], flat["session_id"])
        ...
        if args.report_window_midpoint:
            print(
                f"Window {flat.get('window_id')} mid={mid_ts:.1f} label={final_label} source={label_source}")
    ...
    print(
        f"Saved dataset -> {out_path} ({total} windows, {labeled} labeled, {labeled/total:.1%} coverage)")
